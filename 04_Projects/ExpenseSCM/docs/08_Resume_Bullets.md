...
> 可直接用于简历的项目描述，提供不同长度和侧重点的版本。量化成果部分因无法核实具体数字，已统一采用定性表达（如"显著减少/明显缩短"），避免面试时被追问具体数字却答不上来。
...
- 独立完成系统数据库设计与后端开发，覆盖请购（PR）、采购准备（PPR）、采购订单（PO）、验收、库存、领用、退料、月度成本核算全业务流程，支撑 13 个业务模块、25+ 张核心业务表
...
- 库存事务全程可追溯（事务流水、在库明细、供应层三表可交叉对账），库存差异问题较人工核对阶段显著减少
- 财务月结从人工 Excel 核算改为系统直接出具，月结耗时明显缩短，且不再需要跨系统人工对账
- 审批规则从写死在业务代码里改为 Workflow Engine 配置化管理，审批规则调整的交付周期从按天缩短到按小时
